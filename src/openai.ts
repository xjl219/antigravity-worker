...
export function streamToOpenAI(body:ReadableStream<Uint8Array>,model:string){
  const dec=new TextDecoder(),enc=new TextEncoder(); let buf="",role=false;
  return new ReadableStream<Uint8Array>({start(controller){
    const reader=body.getReader();
    (async()=>{try{
      for(;;){const {done,value}=await reader.read();if(done)break;buf+=dec.decode(value,{stream:true});
        const lines=buf.split(/\\r?\\n/);buf=lines.pop()??"";
        for(const line of lines){let raw=line.trim();if(!raw||raw.startsWith(":"))continue;
          ...
          if(!t)continue; const chunk={id:`chatcmpl-${crypto.randomUUID()}`,object:"chat.completion.chunk",
            created:Math.floor(Date.now()/1000),model,choices:[{index:0,delta:role?{content:t}:{role:"assistant",content:t},finish_reason:null}]};
          role=true;controller.enqueue(enc.encode(sse(chunk)));
        }
      }
      controller.enqueue(enc.encode(sse({id:`chatcmpl-${crypto.randomUUID()}`,object:"chat.completion.chunk",created:Math.floor(Date.now()/1000),model,choices:[{index:0,delta:{},finish_reason:"stop"}]})));
...
export async function toOpenAI(r:Response,model:string){
  ...
  return Response.json({id:`chatcmpl-${crypto.randomUUID()}`,object:"chat.completion",created:Math.floor(Date.now()/1000),model,
